...
{
    public class KineticSerialMatch
    {
        ...
        {
            string sqlCmd = string.Format("select company, parentpartnum, parentserialno, childpartnum, childserialno, datematched from Erp.serialmatch where childpartnum = '{0}' and childserialno = '{1}'",
        ...
        private bool CheckRecordExist()
        {
            ...
            string sqlCmd = string.Format("select * from Erp.serialmatch where childpartnum = '{0}' and childserialno = '{1}'", this.ChildPartNum, this.PreviousChildSerialNo);
            dbAccess.SetQueryCmd(sqlCmd);
            object retVal = dbAccess.GetASingleValue();
            if (retVal != null)
            {
        ...
        {
            string sqlCmd = string.Format("update Erp.serialmatch set childserialno = '{0}', datematched = '{1}' where childpartnum = '{2}' and childserialno = '{3}'",
        ...
        private bool InsertTbl()
        {
            ...
            string sqlCmd = string.Format("insert into Erp.serialmatch (company, parentpartnum, parentserialno, childpartnum, childserialno, datematched) values('AMC', '{0}', '{1}', '{2}', '{3}', '{4}')",
